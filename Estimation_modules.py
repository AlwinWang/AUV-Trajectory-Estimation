...
class VisualEstimation():
    ...
    def get_intrinsics_param(self, dataset):
        ...
        if dataset == "kitti":
            ...
            tum_intrinsics = {
                "tum-1": [318.6, 255.3, 517.3, 516.5],  
                "tum-2": [325.1, 249.7, 520.9, 521.0], 
                "tum-3": [320.1, 247.6, 535.4, 539.2], 
            }
    ...
    def deep_flow_forward(self, in_cur_data, in_ref_data, forward_backward):
        ...
        for i in range(num_forward):
            ...
            kp_ref_best[i*batch_size:(i+1)*batch_size] = batch_kp_cur_best.copy() 
            kp_ref_regular[i*batch_size:(i+1)*batch_size] = batch_kp_cur_regular.copy() 
        ...
        cur_data['kp_best'] = cur_data['kp_best'][cur_data['kp_best'][:,0]!=-1] 
        for i, ref_id in enumerate(ref_data['id']):
            ...
            ref_data['kp_best'][ref_id] = ref_data['kp_best'][ref_id][ref_data['kp_best'][ref_id][:,0]!=-1]
